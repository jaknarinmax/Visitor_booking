<script>
  ...
  export default {
    ...
    selectItem(item) {
      ...
      this.isDropdownOpen = false; // Close the dropdown after selecting an item
    },
    flagImagePath(locale) {
      // Your logic to get the image path based on locale
    }
  ...
  <template>
    ...
    <nav id="nav" class="text-center">
      ...
      <div class="z- 10 max-w-screen-lg xl:max-w-screen-xl block sm:flex sm:justify-between sm:items-center my-6">
        ...
        <div class="flex justify-between items-center   px-0 sm:px-0">
          <!-- Header logos -->
          <div>
            <router-link to="/"><img v-if="theme === 'light'" src="@/assets/images/homejin.png" class="w-36"
                alt="Dark Logo" />
              <img v-else src="@/assets/images/homejin.png" class="w-36" alt="Light Logo" />
            </router-link>
          </div>
        ...
        <div class="hidden sm:flex justify-between items-center flex-col md:flex-row ">
          ...
          <div class="hidden md:block">
            <router-link :to="{ path: '/' }" aria-label="Hire Me Button">
              <Button title="Log in"
  ...
  #nav {
    ...
    /* เปลี่ยนสีพื้นหลังตามที่ต้องการ */
    box-shadow: 10px 14px 16px rgba(0, 0, 0, 0.1);
    /* กำหนดค่าเงา */
  }
  
  /* เพิ่มสีพื้นหลังให้กับโลโก้ */
  #nav img {
    background-color: #ffffff;
    /* เปลี่ยนสีพื้นหลังตามที่ต้องการ */
  }
