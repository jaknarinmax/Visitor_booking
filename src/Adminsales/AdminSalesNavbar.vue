<template>
  <div class="flex h-screen" @click="handleClickOutside">
    ...
    <div ref="sidebar" :class="['transition-all duration-300 ease-in-out', isSidebarExpanded ? 'w-52' : 'w-20', 'bg-gradient-to-b from-gray-800 to-gray-900 text-white flex flex-col' ]" @click.stop>
      ...
      <!-- Logout button -->
      <div class="mt-auto">
        <a @click.prevent="logout" href="#" class="flex items-center px-4 py-4 text-red-400 hover:bg-red-600 hover:text-white transition-all duration-200" :class="{ 'justify-center': !isSidebarExpanded }">
...
export default {
  setup() {
    ...
    const navLinks = [
      { path: '/Adminsales', text: 'Dashboard', icon: LayoutDashboard },
      { path: '/Adminmanagessales', text: 'Adminsales', icon: UserRoundPen },
      { path: '/Adminsalesprofile', text: 'Sales Profile', icon: UserCircle },
      { path: '/Adminsalesbooking', text: 'BookingSales', icon: Calendar },
      { path: '/Adminsalesticket', text: 'Ticket', icon: Ticket },
      { path: '/Adminsalessetting', text: 'Settings', icon: Settings },
    ];
